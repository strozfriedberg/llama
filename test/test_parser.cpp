...
TEST_CASE("startRule") {
  ...
  REQUIRE_NOTHROW(rules = parser.parseRules(lexer.getRuleIndices().size()));
...
TEST_CASE("EmptyStringNoRules") {
  ...
  LlamaLexer lexer = getLexer(input);
  LlamaParser parser(input, lexer.getTokens());
  REQUIRE(parser.parseRules(lexer.getRuleIndices().size()).size() == 0);
}
...
TEST_CASE("inputWithUnrecognizedCharBetweenRulesIsIgnored") {
  ...
  LlamaLexer lexer = getLexer(input);
  REQUIRE(lexer.getRuleIndices() == std::vector<size_t>{0, 7});
  LlamaParser parser(input, lexer.getTokens());
  std::vector<Rule> rules = parser.parseRules(lexer.getRuleIndices().size());
  REQUIRE(rules.size() == 2);
  REQUIRE(rules[0].Name == "myGoodRule");
