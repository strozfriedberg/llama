...
TEST_CASE("buildPropertyNodeSqlClause") {
  ...
  REQUIRE(qb.buildSqlClause(&n) == expected);
}
...
TEST_CASE("buildSqlQueryFromRule") {
  ...
  REQUIRE(qb.buildSqlQuery(hash, rules.at(0)) == "SELECT '" + hash.to_string() + "', Path, Name, inode.Id FROM dirent, inode WHERE dirent.MetaId == inode.Id");
}
...
TEST_CASE("buildSqlQueryFromRuleWithOneNumberFileMetadataCondition") {
  ...
  FieldHash hash = rules.at(0).getHash(parser, hasher);
  REQUIRE(rules.at(0).Name == "MyRule");
  REQUIRE(qb.buildSqlQuery(hash, rules.at(0)) == "SELECT '" + hash.to_string() + "', Path, Name, inode.Id FROM dirent, inode WHERE dirent.MetaId == inode.Id AND Filesize == 30000");
}
...
TEST_CASE("buildSqlQueryFromRuleWithOneStringFileMetadataCondition") {
  ...
  FieldHash hash = rules.at(0).getHash(parser, hasher);
  REQUIRE(rules.at(0).Name == "MyRule");
  REQUIRE(qb.buildSqlQuery(hash, rules.at(0)) == "SELECT '" + hash.to_string() + "', Path, Name, inode.Id FROM dirent, inode WHERE dirent.MetaId == inode.Id AND Created > '2023-05-04'");
}
...
TEST_CASE("buildSqlQueryFromRuleWithCompoundFileMetadataDef") {
  ...
  FieldHash hash = rules.at(0).getHash(parser, hasher);
  REQUIRE(rules.at(0).Name == "MyRule");
  REQUIRE(qb.buildSqlQuery(hash, rules.at(0)) == "SELECT '" + hash.to_string() + "', Path, Name, inode.Id FROM dirent, inode WHERE dirent.MetaId == inode.Id AND (Filesize == 123456 OR (((Created > '2023-05-04' AND Modified < '2023-05-06') AND Name == 'test') AND Path == 'test'))");
}
